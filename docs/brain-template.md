...
the `_brain/` folder with its six memory types, a henxels contract that
...
_brain/
  ...
  conventions/
    index.md             what a convention is, one rule per page
    *.md                 normative memory — standing rules, type: convention (type: decision before format 3)
...
brainpick.toml        shared policy, committed: [bundle] root = "_brain", exclude = ["raw/*"], [brain] format = 3, [half_life.folders] conventions = 0
...
12. One journal file per day, `journals/YYYY-MM-DD.md`, entries newest
    ...
    day; `brainpick migrate --to 3` splits it on its way through format 2.)
...
14. Archived days keep the same shape, untouched. `todo/open.md` is
    ...
    ([To-do lists](todo-lists.md)). A `conventions/` page is
    `type: convention` from format 3 — one standing rule per page, listed
    in `conventions/index.md`, never fading (`[half_life.folders]` pins
    `conventions = 0`); the henxels 0.16 template stamped them
    `type: decision`, which format 3 corrects ([Conventions](conventions.md)).
...
- **The six memory-type folders and their semantics** (plus `raw/`).
  Sufficient by construction: a new memory type is a `type` value or a
  sub-folder, never a further sibling — `conventions/` arrived in format 3
  as a `type` the engine keys on, not a folder it reads. Adding is cheap; renaming is not —
...
3. `brain_overview`, the call every session starts with — lists
   `conventions/`, then `skills/`, before the tree and carries the same
   hint ([MCP tools](mcp-tools.md)).
4. The brain's own `conventions/` and `skills/` — retrievable normative
   and procedural memory that links on to `knowledge/` and to this wiki.
...
`skilltree.md` generation, the overview listing skills ahead of the tree and
...
format 3 — `type: convention` pages listed first
([Conventions](conventions.md)) — and formats 2 (day-per-file journals and
the `todo/` lists, [To-do lists](todo-lists.md)) and 1 alike, and
[brainpick migrate](reference/cli/migrate.md) `--to 3` rewrites either in
deterministic steps: from 1, month files into day files, links onto the
day files, `_todo.md` into `todo/open.md`; from 2, `conventions/*.md`
retyped from `decision` to `convention`, `conventions/index.md` created,
`conventions = 0` added to `[half_life.folders]`; the stamp bumped each
step — a brain whose stamp is behind hears about it at every compile
