...
* [Read-only implants](read-only-implants.md) - mounting a brain you do not control: writes redirect, sync mirrors, push refuses toward the contribution flow
* [Contributing to an implant](contributing-to-an-implant.md) - brain_contribute stages a fix in a worktree against the target's own contract; brain_submit sends it upstream as a pull request
...
* [Skills](skills.md) - procedural memory: recognised by type, linked by depends_on, listed first, boosted in search, never executed — and no skills shipped
* [To-do lists](todo-lists.md) - open work inside the brain: type: todo checklists compile to todos.json, the overview counts them, a hit says how many are open, the archive dates what was done
* [Conventions](conventions.md) - normative memory: type: convention standing rules compile to conventions.json, lead brain_overview and the AGENTS.md report, never fade; brain format 3, migrate --to 3
* [Half-life](half-life.md) - memories fade on purpose: a ranking factor 2^(-age/half_life) on the OKF timestamp, bundle → folder → frontmatter, off by default, nothing deleted
...
* [Update notice](update-notice.md) - agents never check for updates, so the brain tells them: a once-a-day lookup surfaced in the report, the overview and compile output
* [What's new notice](whats-new.md) - a shipped release ledger diffed against the version that last compiled a brain and its stamped format: what changed and what to do, offline, where agents already look
* [The brain ritual](brain-ritual.md) - pull and compile first, consult before grepping, record while working, push last — installed by integrate as a block in AGENTS.md
* [Upgrading a brain](upgrading.md) - the whole migration in one sitting: walk `brainpick whats-new`'s numbered Do next list top to bottom
...
<!-- brainpick:begin index (hash:5892336b) -->
...
- [Contributing to an implant](contributing-to-an-implant.md) — brain_contribute writes and commits a proposed change in a separate working copy (a git worktree, branch contrib/<name>) after passing the target brain's own contract; brain_submit sends it upstream as a pull request — forge CLI, fork remote, or a patch file — never touching the original repository.
- [Conventions](conventions.md) — Normative memory as part of the brain — a type: convention doc is a standing rule the engine lists before anything else: compiled into conventions.json, first in brain_overview and in the AGENTS.md report, pinned by half-life 0; brain format 3 makes the template's conventions/ folder a type the engine recognises, and migrate --to 3 corrects the older stamp.
...
- [Half-life](half-life.md) — Memories fade, and that is a feature — a document's retrieval score is multiplied by 2^(-age/half_life) so a stale page still surfaces but ranks below a fresh one; resolved bundle → folder → frontmatter, off by default, nothing ever deleted.
...
- [MCP tools](mcp-tools.md) — The MCP tools brainpick exposes — overview, search, read, neighbors, write, show, contract, and the gated sync verbs — designed so a 27B model guesses right on the first try.
...
- [Read-only implants](read-only-implants.md) — Mounting a brain you do not control — access = read-only in the registry makes brain_write refuse with a redirect, sync a pure fast-forward mirror, push a refusal that names brain_contribute; your own notes land in your cortex and surface back as annotations.
...
- [The brain ritual](brain-ritual.md) — Four habits that make a brain shared memory rather than a read-only index — pull and compile first, consult before grepping, record while working, commit and push last — installed by brainpick integrate as a fenced block in AGENTS.md and repeated in the Agent Skill.
...
- [To-do lists](todo-lists.md) — Open work as part of the brain — a type: todo doc's checklist lines compile into todos.json, the overview counts them, a search hit says how many are open, and the day-per-file archive makes done an episode with a date; brainpick indexes the list and never edits it.
- [Update notice](update-notice.md) — Agents never check for updates, so the brain tells them — a once-a-day registry lookup whose result surfaces in the AGENTS.md report, brain_overview and compile output, opt-out by config.
- [Upgrading a brain](upgrading.md) — Bring an existing brain to the current release in one sitting — upgrade the engine, migrate the format, install the ritual, retype skills, register — by walking brainpick whats-new's numbered Do next list top to bottom.
- [What's new notice](whats-new.md) — A release ledger every package ships, diffed against the version that last compiled a brain and the brain's stamped format — what changed and what to do about it, surfaced where agents already look, offline and deterministic.
...
- [brainpick integrate](reference/cli/integrate.md) — Install brainpick into an agent harness — the skill, an MCP snippet, the brain report and the brain ritual — additively, with --dry-run.
- [brainpick mcp](reference/cli/mcp.md) — Speak MCP over stdio for agent hosts — the transport the init snippets configure.
- [brainpick migrate](reference/cli/migrate.md) — Rewrite a brain from its stamped format up to a newer one — deterministic, mechanical, writes by default; --dry-run prints the same action list plus a diff and touches nothing.
...
- [brainpick search](reference/cli/search.md) — Search the compiled brain from the terminal — the brain_search tool as a CLI verb, with --mode, --limit and --json.
...
- [brainpick skill](reference/cli/skill.md) — List a brain's skills with their prerequisites and tools, or scaffold a compliant empty skill — type: skill, real timestamp, linked prerequisites, the evaluate-and-improve loop in its body — and compile so the tree already knows it.
...
- [brainpick whats-new](reference/cli/whats-new.md) — What changed in brainpick since this brain was last compiled, and what an agent should do about it — read from the shipped release ledger, offline.
...
- [brain.format](reference/config/brain-format.md) — The brain-format version a bundle follows — 0 (absent) means a plain wiki, 1–3 the brain format of spec/85 at that revision, 3 current; the stamp `brainpick migrate` bumps.
...
- [half_life.default](reference/config/half-life-default.md) — Bundle-wide half-life in days for the ranking factor that fades stale docs — default 0, nothing fades.
- [half_life.folders](reference/config/half-life-folders.md) — Folder → days table for the half-life ranking factor; the longest folder prefix of a doc's path wins.
...
- [models.embedding](reference/config/models-embedding.md) — The T2 embedding backend — kind, endpoint, model and dim — a machine-local table that belongs in brainpick.local.toml.
...
- [update.check](reference/config/update-check.md) — Whether compile and serve may look up the latest published brainpick version once a day — true (default) or false for an air-gapped or private machine.
...
- [brain_overview](reference/mcp/brain-overview.md) — The orientation tool — bundle name, counts, tier availability, the conventions, the skills and the index tree — the progressive-disclosure root, with no required arguments.
...
- [brain_search](reference/mcp/brain-search.md) — Search returning titles and descriptions with a match reason — never full bodies — across auto, keyword, semantic and graph modes.
...
- [Spec: T1 artifacts](reference/spec/t1-artifacts.md) — The deterministic heart — document scanning, link extraction, graph.json (nodes, edges, ghosts, islands, orphans, tags), docs.jsonl, skills.json, todos.json, conventions.json, and the generated index and skill tree.
...
- [Spec: brain format](reference/spec/brain-format.md) — The normative contract for a brain — the fixed _brain/ root, the six memory-type folders, the engine-consumed frontmatter keys and their additive-only policy, inline grounding, the data flow's folder order, the [brain] config section, the brain:// link syntax and the format version with its migration rule.
