...
<!-- brainpick:begin index (hash:028a9adf) -->
...
- [Knowledge graph tier](knowledge-graph-tier.md) — T3 derives an entity/relation layer algorithmically by default (ghosts and tags, no model needed) — LightRAG's LLM extraction is an opt-in second backend behind the same adapter.
...
- [The daemon](daemon.md) — brainpickd — the service that owns every brain's git sync, supervision, deploy keys, and users behind one small control API; every face (desktop app, browser, CLI) is a thin client of it.
...
- [Time machine](time-machine.md) — The history dimension — brainpick distills the bundle's git history into a timeline artifact so you can scrub through the brain's past and watch it grow.
...
- [ADR: LightRAG behind the KGBackend adapter](reference/adr/kgbackend-adapter.md) — Why T3 entity extraction runs through a narrow KGBackend adapter and a neutral JSONL export, so LightRAG is fenced behind an extra and never leaks into consumers.
...
- [models.extraction](reference/config/models-extraction.md) — The chat model that powers T3 extraction and doubles as the brain_write merge resolver — kind, endpoint, model and api_key_env.
- [modules.graph](reference/config/modules-graph.md) — The T3 entity-graph switch — off (default), auto or on.
...
- [Henxel: concept docs carry OKF frontmatter](reference/henxels/concept-frontmatter.md) — The henxel requiring every docs/ concept to be kebab-case markdown with type/title/description frontmatter, type from the allowed set — the one MUST is type.
...
- [Henxel: every link lands](reference/henxels/links-land.md) — The henxel that every relative and root-absolute internal link resolves to a real file — links are the edges of the brain, and a dead link is a ghost.
...
- [Spec: REST API](reference/spec/rest-api.md) — The HTTP surface both servers implement — health, status, graph, docs, search, neighbors, live, timeline, show, guarded writes and asset upload.
...
- [Spec: T3 knowledge graph](reference/spec/t3-kg.md) — A second, extracted view — the neutral export (entities, relations, kg-meta) with normative layout but advisory content, and normative query semantics.
