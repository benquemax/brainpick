...
<!-- brainpick:begin report (hash:257680ff) -->
...
- Counts: 158 docs · 1152 links · 20 tags · 0 orphans · 0 ghosts
- Tiers: t1 fresh · t2 fresh · t3 fresh
- Top hubs (in/out):
  - Configuration reference (reference-config.md) — 41/38
  - Architecture decision records (reference-adr.md) — 25/25
  - CLI reference (reference-cli.md) — 21/21
  - Compile pipeline (compile-pipeline.md) — 34/7
...
- Top similarity gaps:
  ...
  - agent-integrations.md ↔ reference-cli.md — 0.9
  - agent-integrations.md ↔ brain.md — 0.879
  - federation.md ↔ reference-cli.md — 0.879
  - brain-template.md ↔ reference-cli.md — 0.878
- Bundle root: docs
- What's new: brainpick 0.5.0 → 0.6.0: 1 release since this brain was last compiled — run `brainpick whats-new --since 0.5.0`
...
- Similarity gaps stay resolved: link them, or dismiss reviewed pairs (in ./docs) _(warn)_
  ↳ The second route of the two-route linking strategy (see ADR: the similarity gap-detector): T2 vectors joined against T1's link graph surface pairs that read as similar but carry no edge (spec/45). Unlike every other check in this file, this one reads a COMPILE OUTPUT (docs/.brainpick/t1/similarity-gaps.json), not markdown directly — run `brainpick compile` (T2 on) for this to catch drift accurately; a stale or absent artifact degrades silently, never blocks. Dismiss a reviewed-and-rejected pair in docs/similarity-gaps-allowlist.toml.
