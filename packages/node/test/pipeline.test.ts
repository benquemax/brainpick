...
test("fresh compile writes everything", () => {
...
test("edit bumps seq and updates", () => {
  const root = copyBundle();
  runCompile(root);
  writeFileSync(
    join(root, "uusi.md"),
...
test("full recompile matches incremental", () => {
  const root = copyBundle();
  runCompile(root);
  const incremental = readFileSync(join(root, ".brainpick", "t1", "graph.json"));
  const result = runCompile(root, true);
...
test("tier-status-only transition rewrites the manifest without spending a seq", () => {
  ...
  expect(runCompile(root).changed).toBe(false); // and now it settles
});

test("delta carries cause paths and seq after a change", () => {
  const root = copyBundle();
  runCompile(root);
